...
class Express {
  ...
  constructor(options) {
    ...
    if(this.options.logger) {
      this.logger = typeof this.options.logger.get === 'function' ? logger.get(options.name) : logger;
    } else {
  ...
  listen() {
    return new Promise((resolve, reject) => {
      ...
      this.app.listen(port, host, err => {
        ...
        logger.get(this.options.name).info(`${this.options.name} listening on http://${host}:${port}`);
        resolve();
      });
    });
  }
